...
- 画像は `.ph` / `.ph-1` 〜 `.ph-3` クラスを土台とすること（読み込み中や欠損時のフォールバック背景として機能する）。2026-09-02 時点で実素材（Unsplash / Pexels の無料ストック写真）を `assets/images/` 配下に配置し、`<img class="ph ...">` として差し込み済みのため、実素材の差し込みは対象外ではなくなった。新規ページを追加する場合も同様に実写を用意すること。
